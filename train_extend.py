...
def parse():
    ...
    parser.add_argument('--data-path', type=str, default='./data/celeba/img_align_celeba/img_align_celeba')
...
if __name__ == '__main__':
    ...
    del test_dset
    del test_data
    vutils.save_image(fixed_reals, join(
        sample_path, 
        '{:07d}_real.jpg'.format(0)), 
        nrow=8, 
        padding=0, 
        normalize=True, 
        # range=(-1., 1.)
    ...
    load_nimg = max(int(path.split('.')[0]) for path in os.listdir(join(checkpoint_path)) if path.split('.')[0].isdigit())
    print('Loading from nimg {:07d}'.format(load_nimg))
    G = Generator()
    G.load_state_dict(torch.load(join(checkpoint_path, '{:07d}.G.pth'.format(load_nimg)), map_location=device))
    D = Discriminator()
    D.load_state_dict(torch.load(join(checkpoint_path, '{:07d}.D.pth'.format(load_nimg)), map_location=device))
    G_opt = optim.Adam(G.parameters(), lr=args.lr, betas=(args.beta1, args.beta2))
    G_opt.load_state_dict(torch.load(join(checkpoint_path, '{:07d}.G_opt.pth'.format(load_nimg)), map_location=device))
    D_opt = optim.Adam(D.parameters(), lr=args.lr, betas=(args.beta1, args.beta2))
    D_opt.load_state_dict(torch.load(join(checkpoint_path, '{:07d}.D_opt.pth'.format(load_nimg)), map_location=device))
        ...
        if cur_nimg >= tick_start_nimg + args.tick_kimg * 1000 or done:
            ...
            if cur_tick % args.sample_ticks == 0 or done:
                ...
                with torch.no_grad():
                    ...
                    vutils.save_image(
                        samples, 
                        join(sample_path, '{:07d}_fake.jpg'.format(cur_nimg)), 
                        nrow=8, 
                        padding=0, 
                        normalize=True, 
                        # range=(-1., 1.)
